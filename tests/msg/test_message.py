...
async def test_telegram_message_send_raises_on_chunk_failure():
    """TelegramMessage.send() should stop and bubble up chunk send errors."""
...
async def test_telegram_message_send_update_raises_on_chunk_failure(mocked_cs2_update_post):
    """CounterStrikeUpdateMessage.send() (inherited) should bubble up failures."""
    with patch('requests.get') as mocked_get:
        mocked_get.return_value.ok = True
        mocked_get.return_value.url = "https://test.com"
        msg = await TelegramMessageFactory.create(mocked_cs2_update_post)
...
async def test_telegram_message_send_external_raises_on_chunk_failure(mocked_cs2_external_news):
    """CounterStrikeExternalMessage.send() (inherited) should bubble up failures."""
    with patch('requests.get') as mocked_get:
        mocked_get.return_value.ok = True
        mocked_get.return_value.url = "https://test.com"
        msg = await TelegramMessageFactory.create(mocked_cs2_external_news)
